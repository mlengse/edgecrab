...
use edgecrab_tools::lsp_gate::{LspGate, ToolDiagnostic};
...
async fn pull_via_request(
    ...
    drop(guard);
    Ok(filter_tool_diagnostics(diagnostics))
}
...
impl LspGate for EdgecrabLspGate {
    async fn pull_diagnostics(
        ...
        let result =
            tokio::time::timeout(timeout, pull_via_request(ctx, path)).await;
