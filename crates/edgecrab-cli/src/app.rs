...
pub struct App {
    ...
    /// Reasoning picker overlay (activated by `/reasoning` with no args)
    reasoning_selector_active: bool,
    /// Which row is highlighted in the reasoning picker (0=Low,1=Med,2=High,3=Show,4=Hide)
    reasoning_selector_cursor: usize,
    /// Last-set reasoning effort level — cached so the picker can pre-select it
    reasoning_effort_hint: Option<String>,
    /// Personality picker overlay (activated by `/personality` with no args)
    personality_selector_active: bool,
    /// Which row is highlighted in the personality picker
    personality_selector_cursor: usize,
    /// Cached entries for the personality picker [(name, preview_text)]
    personality_selector_entries: Vec<(String, String)>,
    /// Stream picker overlay (activated by `/stream` with no args)
...
impl App {
    ...
    pub fn new() -> Self {
        ...
        let mut app = Self {
            ...
            reasoning_selector_cursor: 1, // default to Medium
            reasoning_effort_hint: None,
            personality_selector_active: false,
        ...
        match result {
            ...
            CommandResult::SetStreaming(mode) => {
                if mode.trim().is_empty() {
                    self.stream_selector_cursor = if self.streaming_enabled { 0 } else { 1 };
                    self.stream_selector_active = true;
                    self.needs_redraw = true;
                } else {
            ...
            CommandResult::SetStatusBar(mode) => {
                if mode.trim().is_empty() {
                    self.statusbar_selector_cursor = if self.show_status_bar { 0 } else { 1 };
                    self.statusbar_selector_active = true;
                    self.needs_redraw = true;
                } else {
    ...
    fn handle_set_reasoning(&mut self, level: String) {
        ...
        let msg = match normalized.as_str() {
            "low" | "medium" | "high" => {
                // Cache the last-set effort so the picker can pre-select it.
                self.reasoning_effort_hint = Some(normalized.clone());
                if let Some(agent) = self.agent.clone() {
                    self.rt_handle.block_on(async {
